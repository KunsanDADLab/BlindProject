...
class ServerSocket:
    ...
    def receiveImages(self): # 이미지를 받는 스레드
        receiveHandler = True # TRUE면 라즈베리파이로 결과를 송신
        try:
            while True:
                ...
                if receiveHandler:
                    ...
                    cv2.imwrite("clinet.jpg", decimg) # 파일저장
                    
                    cv2.imshow("image", decimg) # 이미지 보여주기
                    cv2.waitKey(1) # 0.001초 대기, ms단위임 1000이면 1초, 근데 0이면 무한대기
                    
                    #CASE 1
                    processResult = 'DONE' # 임의로 정의한 값으로 Yolov5 처리값을 전송
                    self.conn.send(processResult.encode('utf-8').ljust(64))
                
                # #CASE 2    
                #     receiveHandler = False
                
                # elif receiveHandler == False:
                #     processResult = '1'
                #     self.conn.send(processResult.encode('utf-8').ljust(64)) # self.sendall or self.conn.sendall or self.socket.sendall
                #     receiveHandler = True
